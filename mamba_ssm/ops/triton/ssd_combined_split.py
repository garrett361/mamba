...
class StatePassingFn(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dstates, dfinal_states):
        states, dA_cumsum_last, initial_states, seq_idx = ctx.saved_tensors
        d_state = states.shape[-1]
        dstates, ddA_chunk_cumsum_last, dinitial_states, states = _state_passing_bwd(
            rearrange(states, "... p n -> ... (p n)"),
            dA_cumsum_last,
            rearrange(dstates, "... p n -> ... (p n)"),
            dfinal_states=rearrange(dfinal_states, "... p n -> ... (p n)"),
            seq_idx=seq_idx,
            has_initial_states=initial_states is not None,
            dstates_dtype=ctx.out_dtype,
            states_dtype=ctx.out_dtype,
        ...
        return dstates, ddA_chunk_cumsum_last, None, None, dinitial_states, None
