...
def get_normed_hidden_states_and_residual(
    norm: nn.LayerNorm | RMSNorm,
    ...
    if not fused_add_norm:
        ...
        hidden_states, residual = layer_norm_fn(
            ...
            norm.bias,
            residual=residual,
            prenorm=True, # prenorm=False causes layer_norm_fn -> tuple[torch.Tensor, torch.Tensor]
            residual_in_fp32=residual_in_fp32,
            eps=norm.eps,
...
def get_normed_hidden_states(
    ...
    # NOTE: @goon - Not sure if residual_in_fp32 is used with prenorm=False, but leaving it for now
    if not fused_add_norm:
        residual = (hidden_states + residual) if residual is not None else hidden_states
        hidden_states = norm(residual.to(dtype=norm.weight.dtype))
    else:
        hidden_states = layer_norm_fn(
            ...
            norm.bias,
            residual=residual,
            prenorm=False, # prenorm=False causes layer_norm_fn -> torch.Tensor
            residual_in_fp32=residual_in_fp32,
            eps=norm.eps,
            is_rms_norm=isinstance(norm, RMSNorm),
        )
